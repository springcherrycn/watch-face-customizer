const texts = {
    'zh-CN': {
        lang: '语言', region: '时区设置', dial: '表盘颜色', bg: '背景图片',
    ...
    'en': {
        lang: 'Language', region: 'Timezone', dial: 'Dial Color', bg: 'Background Image',
        ...
        saveBtn: 'Apply & Close',
        unsupported: 'This language is not supported yet.'
    }
};
...
dialStyleSelect.addEventListener('change', (e) => {
    ...
    if (value === 'random') {
        ...
        const cl = lightness > 50 ? 20 : 80;
        hourHand.style.background = `hsl(${(hue + 180) % 360}, ${saturation}%, ${cl}%)`;
        minuteHand.style.background = `hsl(${(hue + 180) % 360}, ${saturation}%, ${cl}%)`;
        secondHand.style.background = `hsl(${hue}, ${saturation}%, ${cl}%)`;
    } else {
